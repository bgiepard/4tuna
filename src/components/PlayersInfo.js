  ...
  return (
    <div className={`w-full h-full`}>
      ...
      {gameInfo.players.map((player, index) => (
        <div
          className={`flex flex-col pt-1 pb-1 min-w-[90px] px-2 mx-2 bg-[#6D42DA] rounded-[10px] ${gameInfo.currentPlayer === index && '!opacity-100 bg-[#E4BC45] shadow-xl'}
        ...
        >
          ...
          <span
            className={`block p-0 w-4/5 mx-auto text-[12px] leading-[12px] pt-1 pb-0.5 text-center rounded-[3px] text-gray-400 ${gameInfo.currentPlayer === index && '!text-white '} `}
          >
            {player.amount}
          </span>
        </div>
